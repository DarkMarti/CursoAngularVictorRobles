...
var controller = {
    ...
    deleteProject : function(req, res){
        ...
        Project.findByIdAndRemove(projectId, (err, projectRemoved) => {
            if(err) return res.status(500).send({message: 'No se ha podidod borrar el proyecto'});
    ...
    uploadImage: function (req, res){
        ...
        if(req.files){
            ...
            if(fileExt.toLowerCase() == 'png' || fileExt.toLowerCase() == 'jpg' || fileExt.toLowerCase() == 'jpeg' || fileExt.toLowerCase() == 'gif'){
                ...
                if(!projectUpdated) return res.status(404).send({message: 'El proyecto no existe y no se puede subir la imagen'});
